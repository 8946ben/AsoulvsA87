...
export class CodexScene extends Phaser.Scene {
  ...
  private createHeader(): void {
    ...
    this.add.text(760, 58, 'TAB 切换', { fontFamily: 'Microsoft YaHei', fontSize: '12px', color: '#71809a' });
  }
  ...
  private createTabButton(x: number, y: number, label: string, onClick: () => void): Phaser.GameObjects.Text {
    ...
    button.on('pointerover', () => { if (button !== (this.activeTab === 'allies' ? this.allyTab : this.enemyTab)) button.setBackgroundColor('#dff1ed'); });
  ...
  private refreshTabs(): void {
    ...
    const idleColor = '#52667d';
    this.allyTab?.setColor(this.activeTab === 'allies' ? activeColor : idleColor).setBackgroundColor(this.activeTab === 'allies' ? '#4eb3cf' : '#edf7f4');
    this.enemyTab?.setColor(this.activeTab === 'enemies' ? activeColor : idleColor).setBackgroundColor(this.activeTab === 'enemies' ? '#e46f98' : '#edf7f4');
  }
  ...
  private bindCardHover(card: Phaser.GameObjects.Container, bg: Phaser.GameObjects.Rectangle, accent: number): void {
    bg.on('pointerover', () => {
      ...
      bg.setFillStyle(Phaser.Display.Color.Interpolate.ColorWithColor(Phaser.Display.Color.ValueToColor(FRESH.PAPER), Phaser.Display.Color.ValueToColor(accent), 100, 8).color, 0.98);
      bg.setStrokeStyle(2, accent, 0.8);
    });
    bg.on('pointerout', () => {
      this.tweens.add({ targets: card, scale: 1, duration: 120, ease: 'Quad.easeOut' });
      bg.setFillStyle(this.activeTab === 'allies' ? FRESH.PAPER : 0xfff7f5, 0.97);
