...
从发布者处取得 `defenseZJ-Windows-x64-v<版本号>.zip`（例如 `defenseZJ-Windows-x64-v0.1.0.zip`）后：
...
> 不要只把 `defenseZJ.exe` 单独复制出去。程序还需要同目录下的 DLL、`resources` 等文件，必须保留完整的解压目录。

游戏进度保存在 exe 旁的 `save-data/` 文件夹中：每个解压副本独立存档、互不影响；删除该文件夹即可把游戏重置为初始状态。请解压到有写入权限的普通目录。
...
> **https://<服务器地址>/game/**（网页版地址由发布者另行提供，服务器地址不入库）
...
- 资源加载失败：启动时若部分素材下载失败（断网、本地服务器未启动等），会显示「资源加载失败」提示页而不是进入游戏，点击「重试加载」即可；已下载的资源走浏览器缓存，重试很快。
...
主界面左上角「⚙ 设置」打开，内含两个系统级入口：

- **开发者模式**：点击后输入口令 `ftqd` 并按回车。开发者模式只在当前程序会话内生效，可以进入任意关卡，并额外开放尚在开发的肉鸽模式；灵境币与答题券固定按 9999 显示，消费、抽卡入库、重复兑换与答题奖励都不写真实存档，角色进阶仅在本会话内预览（关闭开发者模式或重启后还原），不会改写正常通关进度。再次点击该按钮即可关闭，已启用时设置图标会变为绿色提示。
- **清空所有进度**：点击后需再点一次「再点一次确认清空」才会执行（3 秒内有效）。执行后关卡进度、科技树、角色收藏（灵境币/进阶）、枝江装备（持有/装配/答题券）、图鉴遭遇与肉鸽残局全部回到初始状态，不可恢复；待提交的意见反馈不受影响。
...
3. 生成可运行目录 `out/defenseZJ-win32-x64/`。
4. 将整个运行目录压缩为：

```text
release/defenseZJ-Windows-x64-v<版本号>.zip
```

版本号取自根目录 `package.json` 的 `version` 字段，升级版本只需改那一处。

将这个 ZIP 发给其他 Windows 用户即可。对方完整解压后双击 `defenseZJ.exe`，不需要安装 Node.js。
...
```text
out/defenseZJ-win32-x64/defenseZJ.exe
```
...
3. 解压新生成的 `release/defenseZJ-Windows-x64-v<版本号>.zip`。
4. 双击其中的 `defenseZJ.exe`。
...
- 游戏上线云端网页版：https://<服务器地址>/game/（nginx 静态托管，`npm run build` 产物直出）
...
请退出所有正在运行的 `defenseZJ.exe`，关闭打开在 `out` 或 `release` 目录中的压缩软件窗口，然后重新打包。
